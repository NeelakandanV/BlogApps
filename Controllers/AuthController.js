        ...
        if(!find_User){
            ...
            var transporter = nodemailer.createTransport({
              ...
              var mailOptions = {
                ...
                to: find_User.Email,
                subject: 'Reset your BlogApp Account Password',
                html:`<p>Kindly click the below link or copy and paste the link in your browser to reset your password.<b>Your Link is valid for only 5 minutes</b></p><a href=${link}>Click here to reset Password</a></br><p></p>copy and paste the link in your browser<p>${link}</p><br/><p>With love💙 BlopApps Team</p>`
              };
    ...
    try{
        ...
        if(!find_User){
            ...
            if(find_User.Status=="Verified"){
                ...
                if(find_User.Status=="Banned"){
                    res.status(400).send({message:"User banned by Admin"})
                }
                else{
                    // Generating a random string
                    const String = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
                    ...
                    var transporter = nodemailer.createTransport({
                      ...
                      var mailOptions = {
                        from: 'hari1507hari@gmail.com',
                        to: find_User.Email,
                        subject: 'Verify Your blogApps Account ',
                        html:`<p>Kindly click the below link or copy and paste the link in your browser to verify your blogApps Account.<b>Your Link is valid for only 5 minutes</b></p><a href=${link}>Click here to verify account</a></br><p></p>copy and paste the link in your browser<p>${link}</p><br/><p>With love💙 BlogApps Team</p>`
                      };
...
export const verifyUser =async(req,res)=>{
    try{
        ...
        if(pin===find_User.VerifyPin){
            ...
            if(Math.floor((+new Date())/1000) < verifyToken.exp){
                // Updating New Password
                const changeStatus = await users.updateOne({_id:id},{$set:{Status:"Verified"}})
